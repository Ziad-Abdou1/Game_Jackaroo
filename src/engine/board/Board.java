...
public class Board implements BoardManager {
	...
	private int getPositionInPath(ArrayList<Cell> path, Marble marble) {
		if (path == null)
			return -1; // I added this because validateSteps function
	...
	// //I considered it 4
	// // if steps can be -ve , you as a programmer should set the boolean
	// stepsCanBeNegative to be true
	...
	// if(cur==-1){
	// ArrayList<Cell> sf=getSafeZone(marble.getColour()); //should I handle if
	// it is null?
	// cur=getPositionInPath(sf, marble);
	// if(cur==-1) throw new
	// IllegalMovementException("the marble cannot be moved.");
	// // marble now is in safe zone
	// if(steps==4) throw new
	// IllegalMovementException("marble cannot move backwards in Safe Zone");
	// if(cur+steps>=4) throw new
	// IllegalMovementException("the rank of the card played is too high.");
	// //steps==5 is included here automatically
	...
	// cell.getMarble().getColour()==clrCurrentPlayer){
	...
	// if(distanceToEnd<steps) throw new
	// IllegalMovementException("the rank of the card played is too high.");
	...
	private ArrayList<Cell> validateSteps(Marble marble, int steps)
		...
		if (cur == -1) {
			...
			if (cur == -1)
				throw new IllegalMovementException(
						"the marble cannot be moved.");
			// marble now is in safe zone
			if (steps < 0)
				throw new IllegalMovementException(
						"marble cannot move backwards in Safe Zone");
			if (cur + steps >= 4)
				throw new IllegalMovementException(
						"the rank of the card played is too high."); // steps==5
																		// is
																		// included
																		// here
																		// automatically
		...
		if (distanceToEnd < steps && clrCurrentPlayer == marble.getColour())
			throw new IllegalMovementException(
					"the rank of the card played is too high.");
		...
		if (clrCurrentPlayer != marble.getColour()) {
			for (int i = 0; i <= steps; i++) {
				Cell cell = track.get((cur + i) % 100);
				ans.add(cell);
				if (i != 0 && cell.getMarble() != null
						&& cell.getMarble().getColour() == clrCurrentPlayer) {
					throw new IllegalMovementException(
							"can not bypass or land your own marbles");
				}
			}
			return ans;
		}
	...
	// IllegalMovementException,IOException{
	// ArrayList<Colour> colourOrder=new
	// ArrayList(Arrays.asList(Colour.RED,Colour.GREEN,Colour.BLUE,Colour.YELLOW));
	...
	private void validatePath(Marble marble, ArrayList<Cell> fullPath,
		...
		if (destroy) {
			for (int i = startIdx; i < fullPath.size(); i++) {
				...
				if (c.getCellType() == CellType.BASE
						&& c.getMarble() != null
						&& track.get(getBasePosition(c.getMarble().getColour())) == c) {
			...
			for (int i = startIdx; i < fullPath.size(); i++) {
				Cell c = fullPath.get(i);
				if (c.getCellType() == CellType.ENTRY && c.getMarble() != null
						&& i < fullPath.size() - 1
						&& fullPath.get(i + 1).getCellType() == CellType.SAFE) {
					// throw new
					// IllegalMovementException("Safe Zone Entry: cannot enter safe zone if any marble is in entry");
					if (i < mn) {
		...
		for (int i = 1; i < fullPath.size(); i++) {
			...
			if (c.getCellType() == CellType.SAFE && c.getMarble() != null) {
				// throw new
				// IllegalMovementException("marbles in their Safe Zone are safe from any interference");
				if (i < mn) {
	...
	private void validateSwap(Marble marble_1, Marble marble_2)
		...
		// I assumed that marble_1 is my marble , and marble_2 is my opponent
		// marble
		...
		if (idx1 == -1 || idx2 == -1) {
			throw new IllegalSwapException(
					"One of the marbles is not in the track");
		}
		...
		// //this is not mentioned in the milestone , but rather in the game
		// description
		// //it says that the 2 marbles should not be to the same player
		// if(marble_1.getColour()==marble_2.getColour()){
		// throw new IllegalSwapException();
		// }
		// this will be handled in the boolean
		// validateMarbleColours(ArrayList<Marble> marbles);
	}
	...
	private void validateDestroy(int positionInPath)
		...
		// if(m==null){ //He didn't mention this. I write it myself. because it
		// has no meaning that I destroy an empty cell
		// also it handles the next if condition, in case of m==null , null
		// pointer exception
		// throw new IllegalDestroyException();
		// }
		// if(m==null) throw new
		// IllegalDestroyException("marble is not found in the track"); //I
		// changed my mind. if m==null .
		if (m == null)
			return;
		// I think in real use of this function , if m=null , then postionInPath
		// will =-1. So this condition is not important anymore(and not do what
		// is expected)
		...
		if (m != null) {
			...
			if (m.getColour() == gameManager.getActivePlayerColour())
				throw new CannotFieldException(
						"a marble of same colour exist in your base cell");
			// if(track.get(idxbase)==occupiedBaseCell) throw new
	...
	private void validateSaving(int positionInSafeZone, int positionOnTrack)
		...
		if (positionOnTrack == -1 || positionInSafeZone != -1)
			throw new InvalidMarbleException(
					"the selected marble is not in the track");
	...
	public void moveBy(Marble marble, int steps, boolean destroy)
		...
		// PLEASE: don't use this method inside Game.sendHome(Marble marble)
		// function
	...
	public void destroyMarble(Marble marble) throws IllegalDestroyException {
		...
		// PLEASE: don't use this function inside Game.sendHome(Marble marble)
		// function

		int idx = getPositionInPath(track, marble);
		if (marble.getColour() != gameManager.getActivePlayerColour()) // note
																		// that
																		// this
																		// will
																		// happen
	...
	public void sendToSafe(Marble marble) throws InvalidMarbleException {
		...
		validateSaving(getPositionInPath(sf, marble),
				getPositionInPath(track, marble)); // 0 is any integer , because
													// validateSaving() doesn't
	...
	public ArrayList<Marble> getActionableMarbles() {
		// the marbles on the track are returned , even if it is not my color ,
		// because I can use these marbles if I want to swap with them.
